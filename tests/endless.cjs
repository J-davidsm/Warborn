...
for(const difficulty of ['easy','medium','hard','impossible']){
 ...
 const oldTown=c.settlements[154],oldTerrain=c.terrain[105],survivor=c.units.find(u=>u.team==='PLAYER');c.units.push(c.makeUnit('Soldier','PLAYER',3,19));const doomed=c.units.at(-1).id;
 c.turnNumber=2;run('Endless.advance()');assert.equal(run('Endless.wave'),2);assert.equal(survivor.row,15);assert(!c.units.some(u=>u.id===doomed));assert.equal(c.settlements[164],oldTown);assert.equal(c.terrain[115],oldTerrain);assert(c.units.some(u=>u.team==='AI'&&u.row===0));run('Endless.advance()');assert.equal(run('Endless.wave'),2,'advance only once per round');
