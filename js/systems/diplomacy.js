...
function createTradeProposal(proposer, target, tradeType, offer, request) {
  const proposal = {
    ...
    expiresOn: turnNumber + 3 // Proposals expire after 3 turns
  };
  
  tradeProposals.push(proposal);
  
  // Notify target about the proposal
  const tradeTypeName = TRADE_PROPOSAL_TYPES[tradeType].name;
  let tradeDescription = `${proposer} proposes ${tradeTypeName}`;
  
  if (tradeType === 'RESOURCE_EXCHANGE') {
    const offerStr = formatResourcesDisplay(offer.resources);
    const requestStr = formatResourcesDisplay(request.resources);
    tradeDescription += `\nOffering: ${offerStr}\nWanting: ${requestStr}`;
  }
  
  addAIMessage(target, tradeDescription, 'TRADE_PROPOSAL');
  console.log(`Trade proposal created: ${proposer} → ${target} (${tradeType})`);
  
  return proposal;
}

function acceptTradeProposal(proposalId, acceptingFaction) {
  const proposal = tradeProposals.find(p => p.id === proposalId && p.status === 'PENDING');
  if (!proposal || proposal.target !== acceptingFaction) return false;
  
  // Validate both parties can fulfill the trade
  if (proposal.type === 'RESOURCE_EXCHANGE') {
    if (!hasResources(proposal.proposer, proposal.offer.resources) || 
        !hasResources(proposal.target, proposal.request.resources)) {
      proposal.status = 'CANCELLED';
      addAIMessage(proposal.proposer, 'Trade cancelled - insufficient resources', 'TRADE_RESPONSE');
      return false;
    }
    
    // Execute the resource exchange
    spendResources(proposal.proposer, proposal.offer.resources);
    addResources(proposal.proposer, proposal.request.resources);
    spendResources(proposal.target, proposal.request.resources);
    addResources(proposal.target, proposal.offer.resources);
    
    // Improve diplomatic relations
    modifyTrust(proposal.proposer, proposal.target, 10, 'Successful trade');
  }
  
  proposal.status = 'ACCEPTED';
  addAIMessage(proposal.proposer, `${acceptingFaction} accepted your trade proposal!`, 'TRADE_RESPONSE');
  
  console.log(`Trade accepted: ${proposal.proposer} ↔ ${proposal.target}`);
  return true;
}

function rejectTradeProposal(proposalId, rejectingFaction) {
  const proposal = tradeProposals.find(p => p.id === proposalId && p.status === 'PENDING');
  if (!proposal || proposal.target !== rejectingFaction) return false;
  
  proposal.status = 'REJECTED';
  addAIMessage(proposal.proposer, `${rejectingFaction} rejected your trade proposal.`, 'TRADE_RESPONSE');
  
  // Small diplomatic penalty for rejection
  modifyTrust(proposal.proposer, proposal.target, -2, 'Trade rejection');
  
  console.log(`Trade rejected: ${proposal.proposer} → ${proposal.target}`);
  return true;
}

function evaluateTradeProposal(aiTeam, proposal) {
  if (proposal.type !== 'RESOURCE_EXCHANGE') return false;
  
  const aiResources = getResources(aiTeam);
  const aiPersonality = diplomacy.personalities[aiTeam] ? AI_PERSONALITIES[diplomacy.personalities[aiTeam]] : AI_PERSONALITIES.BALANCED;
  ...
  let acceptanceThreshold = 0.8; // AI wants at least 80% value
  if (aiPersonality.type === 'TRADER') acceptanceThreshold = 0.7;
  if (aiPersonality.type === 'AGGRESSIVE') acceptanceThreshold = 0.9;
  
  // Trust affects willingness to trade
  const trust = getTrust(proposal.proposer, aiTeam);
  if (trust < -20) acceptanceThreshold += 0.2; // Need better deal if low trust
  if (trust > 30) acceptanceThreshold -= 0.1; // Accept worse deals if high trust
  
  const valueRatio = offerValue / requestValue;
  return valueRatio >= acceptanceThreshold;
}
      ...
      <div style="padding:14px;display:grid;grid-template-rows:auto minmax(180px,1fr) auto;gap:10px;max-height:90vh;">
        ...
        <div>
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <button onclick="openTradeProposalInterface()" class="small">Trade Resources</button>
...
function openTradeProposalInterface() {
  if (!currentDiplomacyTarget) {
    showPopup('No Target Selected', 'Please select a faction to trade with first!', 'error');
    return;
  }
  
  // Check if there's a trade treaty with the target
  const hasTradeTreaty = hasTreaty('PLAYER', currentDiplomacyTarget, 'TRADE_AGREEMENT');
  
  if (!hasTradeTreaty) {
    showPopup('Trade Agreement Required', 'You need a Trade Agreement treaty with ' + currentDiplomacyTarget + ' before you can propose trades!', 'error');
    return;
  }
  
  // Close diplomacy menu when opening trade menu
  ...
  const targetResources = getResources(currentDiplomacyTarget);
  
  // Create trade proposal dialog
  const tradeHTML = `
    <div id="tradeProposalModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; justify-content: center; align-items: center; z-index: 2000;" onclick="handleTradeModalClick(event)">
      <div style="background: var(--panel); padding: 20px; border-radius: 8px; max-width: 500px; width: 90%; color: var(--muted);" onclick="event.stopPropagation()">
        <h3 style="color: var(--accent); margin-top: 0;">💰 Trade Proposal to ${currentDiplomacyTarget}</h3>
        ...
        <div style="border: 1px solid var(--muted); border-radius: 4px; padding: 15px; margin: 15px 0;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
            ...
            <div>
              <label style="font-size: 10px;">Materials:</label>
              <input type="number" id="offerMaterials" min="0" max="${playerResources.materials}" value="0" style="width: 100%; padding: 6px; margin-top: 2px; background: white; color: black; border: 2px solid #ccc; border-radius: 4px; font-size: 12px; cursor: text;" onfocus="this.style.borderColor='#f39c12'" onblur="this.style.borderColor='#ccc'">
            </div>
          </div>
        </div>
        ...
        <div style="border: 1px solid var(--muted); border-radius: 4px; padding: 15px; margin: 15px 0;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
            ...
            <div>
              <label style="font-size: 10px;">Materials:</label>
              <input type="number" id="requestMaterials" min="0" max="${targetResources.materials}" value="0" style="width: 100%; padding: 6px; margin-top: 2px; background: white; color: black; border: 2px solid #ccc; border-radius: 4px; font-size: 12px; cursor: text;" onfocus="this.style.borderColor='#f39c12'" onblur="this.style.borderColor='#ccc'">
            </div>
          </div>
        </div>
        
        <div style="display: flex; gap: 10px; margin-top: 20px;">
          <button onclick="submitTradeProposal()" style="flex: 1; padding: 8px; background: var(--accent); color: var(--bg); border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Send Proposal</button>
          <button onclick="closeTradeProposalModal()" style="flex: 1; padding: 8px; background: var(--muted); color: white; border: none; border-radius: 4px; cursor: pointer;">Cancel</button>
        </div>
      </div>
    </div>
  `;
...
function handleTradeModalClick(event) {
  // Only close if clicking on the modal backdrop, not on inputs or content
  const modalContent = event.target.closest('[onclick="event.stopPropagation()"]');
  const isInputField = event.target.tagName === 'INPUT' || event.target.closest('input');
  const isButton = event.target.tagName === 'BUTTON' || event.target.closest('button');
  
  if (!modalContent && !isInputField && !isButton) {
...
function submitTradeProposal() {
  ...
  // Auto-evaluate if it's an AI
  if (currentDiplomacyTarget.startsWith('AI')) {
    setTimeout(() => {
      if (evaluateTradeProposal(currentDiplomacyTarget, proposal)) {
        acceptTradeProposal(proposal.id, currentDiplomacyTarget);
        showPopup('Trade Accepted', `${currentDiplomacyTarget} accepted your trade proposal!`, 'success');
      } else {
        rejectTradeProposal(proposal.id, currentDiplomacyTarget);
        showPopup('Trade Rejected', `${currentDiplomacyTarget} rejected your trade proposal.`, 'info');
      }
      updateUI();
    }, 1000);
  }
